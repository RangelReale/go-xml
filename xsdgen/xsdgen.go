	...
	graph.Flatten(func(i int) {
		...
		for _, el := range b.Elements {
			if _, ok := shadowedElements[el.Name]; !ok {
				newEl := el
				newEl.Doc = fmt.Sprintf("from [%s]", cfg.public(b.Name))
		...
		for _, attr := range b.Attributes {
			if _, ok := shadowedAttributes[attr.Name]; !ok {
				newAttr := attr
				newAttr.Doc = fmt.Sprintf("from [%s]", cfg.public(b.Name))
...
func (cfg *Config) genComplexType(t *xsd.ComplexType) ([]spec, error) {
	...
	if ok {
		// add base type
		bident, err := cfg.exprName(b, NameTypeImpl)
		if err != nil {
			return nil, fmt.Errorf("%s base type %s: %v")
		}
	...
	for _, el := range elements {
		...
		fieldOptional := optional && !el.Plural && cfg.isOptional(el.Type)
		fields = append(fields, gen.StructArg{
			Doc:      el.Doc,
			Name:     name,
			Typ:      base,
		...
		if /*el.Default != "" ||*/ nonTrivialBuiltin(el.Type) {
			...
			overrides = append(overrides, fieldOverride{
				...
				FromType:     cfg.exprNameString(el.Type, NameTypeImpl),
				Tag:          tag,
				ToType:       typeName,
				Type:         el.Type,
			})
	...
	for _, attr := range attributes {
		...
		base, err := cfg.exprName(attr.Type, NameTypeImpl)
		if err != nil {
		...
		fieldOptional := attr.Optional && cfg.isOptional(attr.Type)
		fields = append(fields, gen.StructArg{
			Doc:      attr.Doc,
			Name:     name,
			Typ:      base,
		...
		if /*attr.Default != "" ||*/ nonTrivialBuiltin(attr.Type) {
			...
			overrides = append(overrides, fieldOverride{
				...
				FieldName:    name.(*ast.Ident).Name,
				FromType:     cfg.exprNameString(attr.Type, NameTypeImpl),
				Tag:          tag,
				ToType:       typeName,
				Type:         attr.Type,
			})
	...
	s := spec{
		doc:         t.Doc,
		name:        cfg.publicType(t, NameTypeImpl),
		expr:        expr,
		xsdType:     t,
		helperTypes: helperTypes,
	}
	if t.Abstract {
		s.methods = append(s.methods, gen.Func(cfg.abstractFunction(t)).
			Receiver("t *"+s.name).
			Body(``).
			MustDecl())
	...
	for _, dc := range decodeConfigs {
		...
		case decodeOpDecode:
			if dc.optional {
				...
				_, _ = body.WriteString(`{` + "\n")
				_, _ = body.WriteString(fmt.Sprintf(`decoded, err := t.%s.Decode()`, dc.name) + "\n")
			}
			_, _ = body.WriteString(`if err != nil {` + "\n")
			_, _ = body.WriteString(`return nil, err` + "\n")
			_, _ = body.WriteString(`}` + "\n")
		...
		case decodeOpResolve:
			...
			_, _ = body.WriteString(fmt.Sprintf(`if !t.%s.IsResolved {`, dc.name) + "\n")
			_, _ = body.WriteString(fmt.Sprintf(`return nil, fmt.Errorf("%s is not resolved")`, dc.name) + "\n")
			_, _ = body.WriteString(`}` + "\n")
...
func (cfg *Config) genComplexTypeAbstract(t *xsd.ComplexType) ([]spec, error) {
	...
	if cfg.isDecode {
		valueExpr, err := cfg.exprName(t, NameTypeAbstract)
		if err != nil {
			return nil, err
		}
		...
		_, _ = decodeBody.WriteString(`ret, err := t.ValueDecodeAny()` + "\n")
		_, _ = decodeBody.WriteString(`if err != nil {` + "\n")
		_, _ = decodeBody.WriteString(`return nil, err` + "\n")
		_, _ = decodeBody.WriteString(`}` + "\n")
		_, _ = decodeBody.WriteString(fmt.Sprintf(`if dec, ok := ret.(dec.%s); ok {`, valueTyp) + "\n")
		_, _ = decodeBody.WriteString(`return dec, nil` + "\n")
		_, _ = decodeBody.WriteString(`}` + "\n")
		_, _ = decodeBody.WriteString(fmt.Sprintf(`return nil, fmt.Errorf("value does not implement 'dec.%s'")`, valueTyp) + "\n")

		decoderSpec.methods = append(decoderSpec.methods, gen.Func("Decode").
			Receiver("t *"+decoderSpec.name).
			Returns(fmt.Sprintf("dec.%s", valueTyp), `error`).
			Body(decodeBody.String()).
			MustDecl())
...
func (cfg *Config) genComplexTypeMethods(t *xsd.ComplexType, overrides []fieldOverride) (marshal, unmarshal *ast.FuncDecl, err error) {
	...
	data.Type = cfg.publicType(t, NameTypeImpl)
...
func (cfg *Config) genSimpleListSpec(t *xsd.SimpleType) ([]spec, error) {
	...
	expr, err := cfg.exprName(t.Base, NameTypeImpl)
	if err != nil {
		return nil, err
	}
